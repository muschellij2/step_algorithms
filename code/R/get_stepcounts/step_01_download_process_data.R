...
get_and_join_files =
  function(file) {
    ...
    id = temp$id[1]
    loc = temp$loc[1]
    loc = ifelse(loc == "Semiregular", "SemiRegular", loc)
    ...
    if (!file.exists(here::here("data", "reorganized", "clemson", id_new))) {
      dir.create(here::here("data", "reorganized", "clemson", id_new), recursive = TRUE)
    }
...
# create directory for raw files, put downloaded data into it
if(file.exists(here::here("data", "raw", "MAREA_dataset"))){
  ...
  lapply(marea_list, function(item) {
    ...
    if (!file.exists(here::here("data", "reorganized", "marea", id_new))) {
      dir.create(here::here("data", "reorganized", "marea", id_new), recursive = TRUE)
    }
...
get_and_join_files =
  function(wrist_file) {
    ...
    if (!file.exists(here::here("data", "reorganized", "oxwalk", id))) {
      dir.create(here::here("data", "reorganized", "oxwalk", id), recursive = TRUE)
    }
